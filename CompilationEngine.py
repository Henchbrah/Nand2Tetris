...
class CompilationEngine(object):
    ...
    def __init__(self,tokenizer,table):
    ...
    def compileClass(self):
        ...
        self.write('<class>')           
        self.incrIndent()                     
        kind = self.writeNextToken('class')    
        self.writeNextToken('IDENTIFIER',define=(None,kind))
        self.writeNextToken('{')          
    ...
    def compileClassVarDec(self):
        ...
        self.incrIndent()       
        kind = self.writeNextToken(['field','static'])          
        type = self.writeNextToken(['KEYWORD','IDENTIFIER'])
        self.writeNextToken('IDENTIFIER',define=(type,kind))
    ...
    def compileParameterList(self):
        ...
        while self.lookAhead()[1] <> ')':
            if atLeastOne: 
                ...
                atLeastOne = True
            type = self.writeNextToken(['KEYWORD','IDENDIFIER'])
            self.writeNextToken('IDENTIFIER',define=(type,kind))    
        self.decrIndent()
        self.write('</parameterlist>')
    ...
    def compileSubroutine(self):
        ...
        self.incrIndent()
        kind = self.writeNextToken(['constructor','function','method'])
        self.writeNextToken(['KEYWORD','IDENTIFIER'])
        self.writeNextToken('IDENTIFIER',define=(None,kind))
        self.table.startSubroutine()######### new
        self.writeNextToken('(')
        ...
        self.decrIndent()
        self.write('</subroutine>')
        self.table.endSubroutine() #### new        
    
    def compileVarDec(self):
        self.write('<varDec>')
        self.incrIndent()   
        kind = self.writeNextToken('var')                          
        type = self.writeNextToken(['KEYWORD','IDENTIFIER'])
        self.writeNextToken('IDENTIFIER',define(type,kind))
        while self.lookAhead()[1] <> ';':
            self.writeNextToken(',')
    ...
    def writeNextToken(self,expected=None,define=None):
        if expected <> None:
            self.validate(expected)
        tokType,tok = self.getNextToken() 
        if define:
            (type,kind) = define
            self.table.define(tok,type,kind)
        self.writeToken(tokType,tok,define)
        return tok                                 #### new, in case needed
    def writeToken(self,tokType,tok,define):
        """Writes simple one-line token"""
        if str(tok) in '<>"&':
            tok = specialSymbols[tok]
        if tokType == 'identifier':
            status = 'used'
            type = self.table.getType(tok) 
            kind = self.table.getKind(tok) 
            index = self.table.getIndex(tok)
            if define: 
                status = 'defined'
            self.write('<>%s %s %s %s %s<>' %(status,tok,type,kind,index))
            print tok
            print self.table.classTable
            print self.table.subroutineTable
        else:
